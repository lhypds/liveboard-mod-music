...
import { ROOTS, QUALITIES, TUNING, chordNotes, guitarVoicings, mod12, parseChord } from "./chords";
...
const LABELS = {
  en: {
    ...
    empty: "Choose or enter a chord to see both instruments.", voicing: "Voicing", previous: "Previous voicing", next: "Next voicing",
  ...
  zh: {
    ...
    empty: "选择或输入和弦，即可查看两种乐器的按法。", voicing: "指型", previous: "上一个指型", next: "下一个指型",
  ...
  ja: {
    ...
    empty: "コードを選択・入力すると両方の楽器の押さえ方を表示します。", voicing: "フォーム", previous: "前のフォーム", next: "次のフォーム",
...
function Piano({ chord, labels }: { chord: ChordValue; labels: Labels }) {
  ...
  const drawKey = (midi: number, x: number, black: boolean) => {
    ...
    return <g key={midi}>
      ...
      {note && <text x={x + (black ? 12 : 20)} y={black ? 76 : 129} textAnchor="middle"
        className={root ? styles.lightText : styles.keyText}>{pretty(note.name)}</text>}
      {!black && midi % 12 === 0 && !note && <text x={x + 20} y="129" textAnchor="middle" className={styles.octave}>C{Math.floor(midi / 12) - 1}</text>}
    </g>;
  };
  return <svg className={styles.piano} viewBox="0 0 560 146" role="img" aria-label={`${labels.piano}: ${notes.map(n => `${pretty(n.name)} (${n.midi})`).join(", ")}`}>
...
export default function Chord({ config }: { config: Record<string, unknown> }) {
  ...
  const selected = chord ?? parseChord(saved)!;
  const voicings = chord ? guitarVoicings(chord) : [];
  const storedIndex = typeof comp?.voicing === "number" ? comp.voicing : local.voicing;
  const index = chord?.symbol === parseChord(saved)?.symbol && Number.isInteger(storedIndex) && storedIndex >= 0 && storedIndex < voicings.length ? storedIndex : 0;
  const error = !chord && value.trim().length > 0;
    ...
    <div className={styles.controls}>
      <label className={styles.inputLabel} htmlFor={id}>
        ...
        <input id={id} value={value} placeholder="C, Am, F#7, B♭maj7" autoComplete="off" spellCheck={false}
          aria-invalid={error} aria-describedby={!chord ? `${id}-hint` : undefined}
          onChange={event => setDraft({ value: event.target.value, source: saved })}
          onBlur={() => { if (chord) commit(chord.symbol, index); }}
          onKeyDown={event => {
            if (event.key === "Enter" && chord) { event.preventDefault(); commit(chord.symbol, index); }
            if (event.key === "Escape") setDraft(null);
          }} />
      </label>
      <div className={styles.selectLabel}><span>{t.root}</span>
        <Dropdown value={selected.root} ariaLabel={t.root}
          options={[...new Set([...ROOTS, selected.root])].map(root => ({ value: root, label: pretty(root) }))}
          onChange={root => commit(root + selected.quality)} />
      </div>
      <div className={styles.selectLabel}><span>{t.quality}</span>
        <Dropdown value={selected.quality} ariaLabel={t.quality}
          options={QUALITIES.map((quality, i) => ({ value: quality, label: t.qualities[i] }))}
          onChange={quality => commit(selected.root + quality)} />
      </div>
    </div>
    {!chord ? <p id={`${id}-hint`} role="status" className={error ? styles.error : styles.hint}>{error ? t.error : t.empty}</p> : <>
      ...
      <section className={styles.section}>
        ...
        <div className={styles.legend}><span><i className={styles.rootSwatch} />{t.rootNote}</span><span><i className={styles.toneSwatch} />{t.chordTone}</span></div>
      </section>
      <section className={styles.section}>
        <div className={styles.sectionHead}><h3>{t.guitar}</h3><span>{t.tuning}</span></div>
        <div className={styles.guitarRow}>
          <Guitar chord={chord} voicing={voicings[index]} labels={t} />
          <div className={styles.guitarHelp}>
            ...
            <p>{t.fret}: {Math.min(...voicings[index].frets.filter(fret => fret > 0))}</p>
            <p className={styles.tabular}>{t.frets}<br />{voicings[index].frets.map(fret => fret < 0 ? "×" : fret).join(" · ")}</p>
            <p>{t.open}<br />{t.mute}</p>
            <p>{t.fingers}</p>
            {voicings[index].barres.length > 0 && <p>{t.barre}</p>}
          </div>
        </div>
